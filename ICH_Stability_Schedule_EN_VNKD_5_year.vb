...
' ==========================
' PUBLIC ENTRY MACROS – BUILD SCHEDULE (3 YEARS)
' ==========================

Public Sub ICH_Schedule_EN_3Y_dd_mmm()
    BuildICHStabilitySchedule LANG_EN, "dd-mmm-yyyy", 3
End Sub

Public Sub ICH_Schedule_EN_3Y_ISO()
    BuildICHStabilitySchedule LANG_EN, "yyyy-mm-dd", 3
End Sub

Public Sub ICH_Schedule_VI_3Y_dd_mm()
    BuildICHStabilitySchedule LANG_VI, "dd/mm/yyyy", 3
End Sub

Public Sub ICH_Schedule_VI_3Y_ISO()
    BuildICHStabilitySchedule LANG_VI, "yyyy-mm-dd", 3
End Sub

' ==========================
' PUBLIC ENTRY MACROS – BUILD SCHEDULE (5 YEARS)
' ==========================

Public Sub ICH_Schedule_EN_5Y_dd_mmm()
    BuildICHStabilitySchedule LANG_EN, "dd-mmm-yyyy", 5
End Sub

Public Sub ICH_Schedule_EN_5Y_ISO()
    BuildICHStabilitySchedule LANG_EN, "yyyy-mm-dd", 5
End Sub

Public Sub ICH_Schedule_VI_5Y_dd_mm()
    BuildICHStabilitySchedule LANG_VI, "dd/mm/yyyy", 5
End Sub

Public Sub ICH_Schedule_VI_5Y_ISO()
    BuildICHStabilitySchedule LANG_VI, "yyyy-mm-dd", 5
End Sub
...
Private Sub BuildICHStabilitySchedule( _
    ...
    ByVal dateFmt As String, _
    ByVal shelfLifeYears As Long)
    ...
    ' Long-term depends on shelf-life years
    ' 3-year:  0,3,6,9,12,18,24,36
    ' 5-year:  0,3,6,9,12,18,24,36,48,60
    If shelfLifeYears >= 5 Then
        longTermMonths = Array(0, 3, 6, 9, 12, 18, 24, 36, 48, 60)
    Else
        longTermMonths = Array(0, 3, 6, 9, 12, 18, 24, 36)
    End If
    
    ' Accelerated and Intermediate usually unchanged
    acceleratedMonths = Array(0, 3, 6)
    intermediateMonths = Array(0, 6, 9, 12)
    
    ' --- Labels by language ---
    If lang = LANG_VI Then
        ' Vietnamese (khong dau, Title Case)
    ...
    ' --- Finished message ---
    If lang = LANG_VI Then
        MsgBox "Da tao lich do on dinh ICH cho han dung " & _
               shelfLifeYears & " nam, bat dau tu ngay " & _
               Format(startDate, dateFmt) & ".", vbInformation
    Else
        MsgBox "ICH stability schedule for " & shelfLifeYears & _
               "-year shelf life created from start date " & _
               Format(startDate, dateFmt) & ".", vbInformation
    End If
End Sub
...
Private Function IsWorkday(ByVal d As Date) As Boolean
    Dim wd As Long
    wd = Weekday(d, vbMonday)  ' Monday = 1, Sunday = 7
...
Private Function RecommendedPullDate( _
    ...
    ' Use target if it is a workday within the window
...
Private Sub CheckDueSamples_ICH_Generic(ByVal lang As String)
    ...
    ' Try to read date format from E2
